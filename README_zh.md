# 🛸 DJIToGoogleDrive (DJI 一键导入 Google Drive)
...
- 🌐 **原生中英多语言自适应与动态切换 (i18n & Localization)**
  - 自动识别 macOS 操作系统主语言（默认匹配中文或英文）；
  - 偏好设置窗口内建中英语言切换器，支持实时界面响应式无缝切换。
...
DJIToGoogleDrive/
├── Sources/
│   ├── DJIToDriveApp/      # macOS MenuBar 原生宿主、SwiftUI 控制面板、多语言与状态流
...
脚本将自动通过 SPM 编译 Release 版本，组装成原生 `DJIToGoogleDrive.app` 并部署到您的桌面上。双击打开即可静默常驻在右上角菜单栏！
